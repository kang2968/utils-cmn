...
public class IEFileUtils {
    ...
    public static boolean writeToFile(String content, final File destFile, final boolean append, final boolean deleteAfterFailed) throws IOException{
        return writeToFile(content, "UTF-8", destFile, append, deleteAfterFailed);
    }
    
    public static boolean writeToFile(String content, String encoding, final File destFile, final boolean append, final boolean deleteAfterFailed) throws IOException {
        if (!createFile(destFile, !append)) {
            return false;
        }
        ...
        try(BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(
                new FileOutputStream(destFile, append), encoding))) {
            writer.write(content);
            writer.flush();
            result = destFile.exists();
        } finally {
            if ((!result) && deleteAfterFailed) {
                try {
                    destFile.delete();
